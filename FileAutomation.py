...
code_folder = "//smbhome.uscs.susx.ac.uk/cb792/Documents/Downloaded Code"
py_folder = "//smbhome.uscs.susx.ac.uk/cb792/Documents/Downloaded Code/Python"
java_folder = "//smbhome.uscs.susx.ac.uk/cb792/Documents/Downloaded Code/Java"
...
if not exists(py_folder):
    os.mkdir(py_folder)
if not exists(java_folder):
...
if __name__ == "__main__":
    ...
    def on_modified(event):
        ...
        if event.src_path.endswith('.mdl'):
            try:
        ...
        if event.src_path.endswith('.py'):
            try:
                shutil.copy(event.src_path, py_folder)
                os.remove(event.src_path)
            except EnvironmentError:
                return
        if event.src_path.endswith('.java'):
            try:
                shutil.copy(event.src_path, java_folder)
                os.remove(event.src_path)
            except EnvironmentError:
                return
